...
io.on('connection', (socket) => {
  if (nUser < 3) {
	...
	socket.on('start', (user) => {
		...
		let chip = JSON.stringify(fichaR);
		chip = chip.charAt(2) + "," + chip.charAt(4);
	...
	socket.on('turn', (user) =>{
		...
		io.emit('showDomino', 'System', "It is " + playerTurn + " Turn,  your turn is: "+usuarios[user].turno);
	});
